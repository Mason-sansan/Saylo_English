...
async function chatJson({ system, user, temperature = 0.4, signal, model }) {
...
async function generateRandomScenarioPack({ level, duration }) {
  ...
  const raw = await chatJson({ system, user, temperature: 0.95 });
...
async function runTurn(sessionId, userText) {
  ...
  if (LLM_API_KEY) {
    ...
    assistant = await chatJson({ system, user });
  } else {
...
async function finishSession(sessionId) {
  ...
  let report = wrapQuickReportWithLegacy(quick, metrics, transcript);
